...
def soundSense(startDateTime, hostIP, BASE_PORT, streaming = True, logging = True):
    ...
    while True:
        if iterations >= FILE_LENGTH:
            ...
            audioMessage.append(str("{0:.3f}".format(sumAudio)))
            startDateTime = rNTPTime.sendUpdate(server_address, audioMessage, 5)
            ...
            startDateTime = rNTPTime.sendUpdate(server_address, doorMessage, 5)
            
            tempMessage = []
            tempMessage.append("Temperature")
            tempMessage.append(str("{0:.3f}".format(sumTemp)))
            startDateTime = rNTPTime.sendUpdate(server_address, tempMessage, 5)
