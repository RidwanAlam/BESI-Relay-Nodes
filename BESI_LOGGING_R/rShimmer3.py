...
def shimmerSense(startDateTime, hostIP, BASE_PORT, ShimmerID, ShimmerID2, streaming = True, logging = True):
    ...
    startDateTime = rNTPTime.sendUpdate(server_address, 0, " Connected to {} ".format(connID),0,0, 5)
	...
	if iterations >= FILE_LENGTH:
			startDateTime = rNTPTime.sendUpdate(server_address, iterations, sensorMessage, 0,0,0,0, 5)
			iterations = -1
		...
		rNTPTime.sendUpdate(server_address, iterations, sensorMessage, 0,0,0,0, 5)
	...
	# if an exception is raised receiving data or connection is lost (streamingError == 1) try to reconnect
	    ...
	    if streamingError == 0:
		    ...
		    ferror.write("Connection Lost from {}. Time: {}\n".format(connID, datetime.datetime.now()-startTime))
       		    startDateTime = rNTPTime.sendUpdate(server_address, 0, " Disconnected from {}".format(connID), 0,0, 5)
